...
@if(isset($data) && count($data) > 0)
    <div class="block-product mt-fix-20">
        ...
        <span>
            ...
            <p class="text-title" >{{ $title??'Nick ngon giá rẻ' }}</p>
                    ...
                    @foreach($data as $key => $item)
                        <div class="swiper-slide" >
                            <a href="/mua-acc/{{ isset($item->custom->slug) && $item->custom->slug != '' ? $item->custom->slug :  $item->slug }}">
                                ...
                                <div class="item-nick-name text-title limit-1" style="overflow: hidden;text-overflow: ellipsis;display: -webkit-box;-webkit-box-orient: vertical;word-break: break-all;-webkit-line-clamp: 1;line-clamp: 1">{{ isset($item->custom->title) ? $item->custom->title :  $item->title }}</div>
                            </a>
                        </div>
