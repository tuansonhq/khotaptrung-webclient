                            ...
                            <div class="inner">
                                <div class="item-logo mb-2 d-flex align-items-center">
                                    @foreach ($item->order_detail as $order_detail)
                                        @if ($order_detail->module == "telecom")
                                            Thẻ {{ $order_detail->title }}
                                        @endif
                                    @endforeach
                                </div>
                                <div class="mb-2 d-flex justify-content-between">
                                    ...
                                    <div>
                                        <td>
                                            @foreach ($item->order_detail as $order_detail)
                                                @if ($order_detail->module == "amount")
                                                    {{ formatPrice((int)$order_detail->title) }} đ
                                                @endif
                                            @endforeach
                                        </td>
                                    </div>
                                ...
                                <div class="mb-2 d-flex justify-content-between">
                                    <div class="text-secondary">Số lượng</div>
                                    <div>
                                        <td>
                                            @foreach ($item->order_detail as $order_detail)
                                                @if ($order_detail->module == "quantity")
                                                    {{ $order_detail->title }} thẻ
                                                @endif
                                            @endforeach
                                        </td>
                                    </div>
                                ...
                                <div class="mb-2 d-flex justify-content-between">
                                    ...
                                    <div>
                                        <b class="text-danger">{{ config('module.store-card.status.'.$item->status ) }}</b>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="inner">
                                ...
                                {{-- <div class="mb-2">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="text" class="form-control border-end-0" placeholder="" value="{{  App\Library\Helpers::Decrypt($card->pin,config('module.charge.key_encrypt')) }}" aria-label="">
                                        <span class="input-group-text bg-transparent text-secondary"><i class="fa fa-copy" style="cursor: pointer" data-id="{{  App\Library\Helpers::Decrypt($card->pin,config('module.charge.key_encrypt')) }}"></i></span>
                                    </div>
                                </div>
                                <div class="mb-2 d-flex justify-content-between">
                                    <div class="text-secondary">Số seri</div>
                                    <div>
                                        {{  App\Library\Helpers::Decrypt($card->serial,config('module.charge.key_encrypt')) }}
                                    </div>
                                </div> --}}
                                <div class="mb-2 d-flex justify-content-between">
                                    <div class="text-secondary">Mệnh giá</div>
                                ...
                                <div class="mb-2 d-flex justify-content-between">
                                    <div class="text-secondary">Trạng thái</div>
                                    <div>
                                        <b class="text-success">{{ config('module.store-card.status.'.$item->status ) }}</b>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="item-content">
                            <div class="inner">
                                ...
                                <div class="mb-2 d-flex justify-content-between">
                                    <div class="text-secondary">Trạng thái</div>
                                    <div>
                                        <b class="text-warning">{{ config('module.store-card.status.'.$item->status ) }}</b>
                                    </div>
                                </div>
                            </div>
                        </div>
...
<div class="row">
    <div class="col-md-12 left-right justify-content-end">
        <div class="d-flex justify-content-between align-items-md-center flex-column flex-md-row mt-2 border-top pt-3">
            <div class="text-secondary ml-3 ml-md-0">
            ...
            <nav class="page-pagination mr-3 paginate__v1_storecard paginate__v1_mobie frontend__panigate">
                @if(isset($data))
                    @if($data->total()>1)
